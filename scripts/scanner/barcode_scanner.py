...
def add_to_reminders(item: str) -> bool:
    """Eintrag in Apple Reminders hinzufügen.

    Nutzt osascript argv – kein String-Escaping nötig, funktioniert mit
    allen Sonderzeichen im Produktnamen (Anführungszeichen, Umlaute, etc.).
    """
    script = """
on run argv
...
"""
    ...
    if result.returncode != 0:
        print(f"  ⚠️  Reminders-Fehler: {result.stderr.strip() or 'Keine Berechtigung?'}")
        print(f"  → Systemeinstellungen → Datenschutz → Automatisierung → Terminal → Reminders ✓")
